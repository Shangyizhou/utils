...
/**
 * Appender：调用 setAppender 输出到不同的 Appender 里，可以是终端，可以是文件，可以是流
 * layout：输出格式 setLayout设置输出格式
 * category：像是一个树状图一样的，我们继承不同的节点就可以打印不同的格式，分模块打印
 * 
 * log4cpp 可以支持 C 语言风格和 C++ 风格
*/
