...
#define WRITE_LINE_NUM  100000L     // 十万行数据
#define BUF_LINE_NUM    1000L       // 一千字节缓冲区
#define FLUSH_LINE_INTERVAL  1000L  // 刷新间隔：1s
...
void fwrite_test(int64_t write_line_num, int64_t buf_line_num, int64_t flush_interval) 
{
    ...
    for(int i = 0; i < write_line_num; i++) {
        ostringstream oss;
        oss << "NO." << i << " Root Error Message!\n";        // 47字节 * 10000 = 470000=470k
        buf_string.append(oss.str());

        // 不断累加字符串到 buf_string 中
        // 每一千行调用一次 fwrite
        if(++buf_count >= buf_line_num)   {
            buf_count = 0;
            buf_size = buf_string.size();
            fwrite(buf_string.c_str(), 1, buf_string.size(), file); // 按 1 字节为单位写入所有数据到文件
            buf_string.clear();
        }
        // 超过一千次，刷新一次
...
void write_test(int64_t write_line_num, int64_t buf_line_num, int64_t flush_interval) 
{
    ...
    for(int i = 0; i < write_line_num; i++) {
        ...
        if(++buf_count >= buf_line_num) {
            buf_count = 0;
            buf_size = buf_string.size();
            write(file, buf_string.c_str(), buf_string.size()); // 用户缓冲区写入内核缓冲区
            buf_string.clear();
        }
        if(++flush_count >= flush_interval) {
            flush_count = 0;
            fsync(file); // 内核缓冲区写入磁盘物理设备
        }
    }
